...
- **2026-06-08** 🐳 **원-풀 Docker 배포 + Gemini 3.x 멀티턴 수정**: 이제 저장소를 클론하거나 로컬 빌드 없이 전체 스택을 실행할 수 있습니다——릴리스 시 CI workflow가 멀티아키(`amd64` + `arm64`) 이미지를 **GHCR**에 게시하고, 새 `docker-compose.prod.yml`이 `ghcr.io/hkuds/vibe-trading`을 직접 풀합니다(README의 **Path E**, Synology NAS에서 검증)([#187](https://github.com/HKUDS/Vibe-Trading/pull/187), [#183](https://github.com/HKUDS/Vibe-Trading/issues/183) 종료, @KaiLuettmann 님 감사합니다). 이번엔 **Gemini 3.x 사고 모델 수정도 완성**했습니다: 6/05의 왕복([#176](https://github.com/HKUDS/Vibe-Trading/pull/176))은 인메모리 히스토리만 다뤘지만, 실제 agent loop는 히스토리를 OpenAI 형식 dict로 재생하며 LangChain이 요청 구성 전에 도구 호출별 `thought_signature`를 버렸기 때문에 멀티턴 도구 호출이 여전히 `missing thought_signature`로 400을 냈습니다. 이제 `invoke`와 `stream`이 공유하는 단일 길목 `_convert_input`에서 다시 부착됩니다(병렬 호출——N개 중 첫 번째만 서명됨——도 포함)([#184](https://github.com/HKUDS/Vibe-Trading/pull/184), @ngoanpv 님 감사합니다).
