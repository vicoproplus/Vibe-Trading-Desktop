...
function formatLeverage(leverage: MandateProfile["leverage"], t: (key: NestedKeyOf<Translation>) => string): string {
  if (typeof leverage === "number") {
    return leverage <= 1 ? t("chat.runner.noLeverage") : `${leverage}× leverage`;
  }
  const lowered = leverage.toLowerCase();
  return lowered === "none" || lowered === "" ? t("chat.runner.noLeverage") : leverage;
}
    ...
    >
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          ...
          title={t("chat.mandate.adjustTitle")}
        >
          <SlidersHorizontal className="h-3 w-3" />
          {t("chat.mandate.adjust")}
        </button>
      </div>

      <dl className="mt-2 grid grid-cols-2 gap-x-3 gap-y-1.5 text-[11px]">
        <div className="col-span-2">
          <dt className="text-muted-foreground">{t("chat.mandate.universe")}</dt>
          <dd className="font-medium text-foreground">{formatUniverse(profile.universe)}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.mandate.maxOrder")}</dt>
          <dd className="font-mono font-medium text-foreground">{formatUsd(profile.max_order_usd)}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.mandate.dailyCap")}</dt>
          <dd className="font-mono font-medium text-foreground">{profile.daily_trade_cap} trades/day</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.mandate.leverage")}</dt>
          <dd className="font-medium text-foreground">{formatLeverage(profile.leverage, t)}</dd>
        </div>
        <div>
          <dt className="text-muted-foreground">{t("chat.mandate.instruments")}</dt>
      ...
      {adjusting ? (
        <div className="mt-3 grid gap-2">
          <input
            ...
            placeholder={t("chat.mandate.adjustPlaceholder")}
        ...
        >
          ...
          {busy ? t("chat.mandate.committing") : t("chat.mandate.commit", { label: profile.label })}
        </button>
      )}
    </div>
  );
...
export const MandateProposalCard = memo(function MandateProposalCard({ proposal, committed, onAdjust }: Props) {
  ...
  const handleCommit = useCallback(
    async (ordinal: number) => {
      ...
      if (!broker) {
        toast.error(t("chat.mandate.mandateMissingError"));
        return;
      }
      ...
      try {
        ...
        setBusyOrdinal(null);
        toast.error(error instanceof Error ? error.message : t("chat.mandate.commitFailedError"));
      }
    },
        ...
        <div className="flex-1 min-w-0">
          <span className="inline-flex max-w-full flex-wrap items-center gap-1.5 rounded-lg bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-600 dark:text-emerald-400">
            ...
            <span className="shrink-0">
              {committed.selected_ordinal != null ? t("chat.mandate.active", { ordinal: committed.selected_ordinal }) : t("chat.mandate.activeNoOrdinal")}
            </span>
            {maxOrder != null && (
            ...
            {expires && (
              <span className="shrink-0 text-[10px] text-muted-foreground">
                · {t("chat.mandate.expires")} {expires.toLocaleDateString()}
              </span>
            )}
          </span>
        </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-foreground">
              {isReauth ? t("chat.mandate.reauthTitle") : t("chat.mandate.title")}
            </p>
